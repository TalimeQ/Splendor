...
void AChatHUD::DrawHUD() {
	...
	if (MyPC->WasInputKeyJustPressed(EKeys::Enter))
		if (MyUIWidget.IsValid() && MyUIWidget->ChatInput.IsValid()) FSlateApplication::Get().SetKeyboardFocus(MyUIWidget->ChatInput); // When the user presses Enter he will focus his keypresses on the chat input bar 
}
...
void AChatHUD::AddMessageBP(const int32 Type, const FString& Username, const FString& Text, const bool Replicate) {
	...
	if (newmessage.Type > 0)
		if (Replicate)
		{
			ASplendorGameState* MyPS = Cast<ASplendorGameState>(MyPC->PlayerState);
